...
class ADConv(nn.Module):
    ...
    def forward(self, x):
        ...
        pooled_inputs = F.adaptive_avg_pool2d(x, 1)
        Expert = pooled_inputs.flatten(1)
        routing_weights = self.routing(Expert)
        Alpha = torch.sigmoid(routing_weights)
        x = self.cond_conv(x, Alpha)
        return x
