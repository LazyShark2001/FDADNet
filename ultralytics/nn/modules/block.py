...
class SEM(nn.Module):
    def __init__(self, dim, a=16, b=16, c_h=16, c_w=16, sigma=60):
        super().__init__()
    ...
    def forward(self, x):
        ...
        rows, cols = x1.shape[2], x1.shape[3]
        crow, ccol = rows // 2, cols // 2
        sigma = self.sigma
        x_yzq = np.linspace(-ccol, ccol, cols)
        y_yzq = np.linspace(-crow, crow, rows)
        x_yzq, y_yzq = np.meshgrid(x_yzq, y_yzq)
        gaussian_filter = np.exp(-(x_yzq ** 2 + y_yzq ** 2) / (2 * sigma ** 2))
        gaussian_filter = torch.tensor(gaussian_filter, dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(x.device)
        gaussian_filter_complex = torch.complex(gaussian_filter, torch.zeros_like(gaussian_filter))
        x1[..., :, :] = x1[..., :, :] * gaussian_filter_complex
        x1 = torch.fft.irfft2(x1, s=(c, b), dim=(2, 3), norm='ortho').permute(0, 2, 1, 3).to(x.dtype)
        ...
        x2 = x2 * b_weight
        rows, cols = x2.shape[2], x2.shape[3]
        crow, ccol = rows // 2, cols // 2
        sigma = self.sigma
        x_yzq = np.linspace(-ccol, ccol, cols)
        y_yzq = np.linspace(-crow, crow, rows)
        ...
        gaussian_filter = torch.tensor(gaussian_filter, dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(x.device)
        gaussian_filter_complex = torch.complex(gaussian_filter, torch.zeros_like(gaussian_filter))
        x2[..., :, :] = x2[..., :, :] * gaussian_filter_complex
        x2 = torch.fft.irfft2(x2, s=(c, a), dim=(2, 3), norm='ortho').permute(0, 2, 3, 1).to(x.dtype)
        x3 = torch.fft.rfft2(x3.float(), dim=(2, 3), norm='ortho')
        ...
        gaussian_filter_complex = torch.complex(gaussian_filter, torch.zeros_like(gaussian_filter))
        x1[..., :, :] = x1[..., :, :] * gaussian_filter_complex
        x3 = torch.fft.irfft2(x3, s=(a, b), dim=(2, 3), norm='ortho').to(x.dtype)
        x4 = self.dw_conv(x4)
...
class MFDW(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, e=0.5):
        ...
        self.m = nn.ModuleList(SEM(self.c) for _ in range(n))
